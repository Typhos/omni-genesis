...
export default class InventoryDisplay extends Component {
  render() {
    const {
      shop,
      shop: { name, size, atmosphere, shopType, staff, inventory },
    } = this.props;
      ...
      <Display>
        ...
        <div className='shopInfo'>
          <p className='displayLayout__description'>
            {name} is a {size}, {atmosphere} {shopType}.
          </p>
          <p className='displayLayout__description'>
            This {shopType} is owned by {staff[0].name.displayName}, the {staff[0].age} year old{" "}
            {staff[0].race} {staff[0].occupation}.
          </p>
        </div>
        <Tabs>
          <div label='Staff'>
