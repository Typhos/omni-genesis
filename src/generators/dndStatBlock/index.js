...
export default class StatBlock {
  constructor(race, subRace, cr) {
    ...
    if (!cr) {
      ...
      this.defensiveCR = CrData.cr[Utils.randomInt(0, 6)]._cr;
    ...
    this.cr = !cr ? CrData.cr[Utils.randomInt(0, 6)]._cr : cr;
  ...
  getAtkDmg() {
    ...
    if (this.cr >= 10) {
      num = 3;
      arr = threeAtkOpt[Utils.randomArrayIndex(threeAtkOpt)];
      dmg = Math.round(arr[0] * ((arr[1] + 1) / 2) + arr[2]);
    } else if (this.cr >= 1) {
      num = 2;
      arr = twoAtkOpt[Utils.randomArrayIndex(twoAtkOpt)];
      dmg = Math.round(arr[0] * ((arr[1] + 1) / 2) + arr[2]);
    } else {
      num = 1;
      arr = sAtkOpt[Utils.randomArrayIndex(sAtkOpt)];
      dmg = this.dpr;
    }
