...
export default class StatBlock {
  ...
  getLanguages(race) {
    ...
    const standardLanguages = Languages.language
      .filter( lang => lang.source === "PHB" && lang.type !== "secret" );

    const extraLangOptions = standardLanguages      
      .filter( lang => {
  ...
  splitCr (desiredCR) {
    ...
    const result = avgOptsArray[Utils.randomArrayIndex( avgOptsArray.length)];
